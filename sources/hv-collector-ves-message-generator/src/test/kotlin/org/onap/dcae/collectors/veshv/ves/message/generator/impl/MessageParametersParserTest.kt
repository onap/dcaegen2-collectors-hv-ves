        ...
        given("parameters json array") {
            ...
            on("invalid parameters json") {
                it("should verify messageAmount") {
                    val result = cut.parse(nonNumberMessageAmountParameters())

                    when(result) {
                        is Either.Left -> assertThat(result.a).isInstanceOf(ParsingException::class.java)
                        else -> fail("parsing should fail")
                    }
                }

                it("should verify messageType") {
                    val result = cut.parse(missingMessageTypeParameters())
                    when(result) {
                        is Either.Left -> assertThat(result.a).isInstanceOf(ParsingException::class.java)
                        else -> fail("parsing should fail")
                    }
