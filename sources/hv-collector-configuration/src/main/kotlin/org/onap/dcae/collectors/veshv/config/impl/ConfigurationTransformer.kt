...
internal class ConfigurationTransformer {
    ...
    private fun createSecurityConfiguration(partial: ValidatedPartialConfiguration): SecurityConfiguration =
            SecurityConfiguration(
                    partial.mapBinding {
                        ImmutableSecurityKeys.builder()
                                .keyStore(ImmutableSecurityKeysStore.of(Paths.get(it.keyStoreFile.bind())))
                                .keyStorePassword(Passwords.fromPath(Paths.get(it.keyStorePasswordFile.bind())))
                                .trustStore(ImmutableSecurityKeysStore.of(Paths.get(it.trustStoreFile.bind())))
                                .trustStorePassword(Passwords.fromPath(Paths.get(it.trustStorePasswordFile.bind())))
                                .build()
                    }
            )
