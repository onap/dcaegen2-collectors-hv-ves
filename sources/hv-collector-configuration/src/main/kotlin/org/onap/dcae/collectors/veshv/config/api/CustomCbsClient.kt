...
class CustomCbsClient(private val cbsClientMono: Mono<CbsClient>,
                      ...
                      retrySpec: Retry<Any> = Retry.any<Any>()
                              .retryMax(MAX_RETRIES)
                              .fixedBackoff(Duration.ofSeconds(10L))
                              .jitter(Jitter.random())) {

    private val requestInterval = AtomicReference<Duration>(Duration.ZERO)
    private val firstRequestDelay = AtomicReference<Duration>(Duration.ofNanos(102L))
    ...
    fun updates(cbsRequest: CbsRequest, firstRequestDelay: Duration, requestInterval: Duration): Flux<JsonObject> {
    ...
    fun updateCurrentCbsInterval(intervalUpdate: Duration) {
