    ...
    describe("Configuration provider") {
        ...
        given("configuration is never in cbs") {
            ...
            val configProvider = constructConfigurationProvider(
                    emptyCustomCbsClient(cbsClientMock, configStateListener),
                    configStateListener)
        ...
        given("valid configuration from cbs") {
            ...
            on("new configuration") {
                whenever(customCbsClientMock.updates(any(), eq(firstRequestDelay), eq(requestInterval)))
            ...
            on("new configuration with changed interval") {
                whenever(customCbsClientMock.updates(any(), eq(requestInterval), eq(firstRequestDelay)))
                        .thenReturn(Flux.just(cbsIntervalChangedConfiguration))
                it("should use received configuration") {

                    StepVerifier.create(configProvider().take(1))
                            .consumeNextWith {
                                assertThat(it.requestIntervalSec.orNull()).isEqualTo(5L)
                            }.verifyComplete()
                }
            }
...
private fun emptyCustomCbsClient(cbsClientMock: CbsClient, configStateListener: ConfigurationStateListener) =
        CustomCbsClient(Mono.just(cbsClientMock), configStateListener, { mapOf("k" to "v") })
