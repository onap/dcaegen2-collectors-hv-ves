...
internal object ConfigurationModuleIT : Spek({
    describe("configuration module") {
        ...
        given("configuration module") {
            val sut = ConfigurationModule(mockedConfigStateListener, Mono.just(mockedCbsClient))
            on("new interval change") {
                whenever(mockedCbsClient.get(any()))
                        .thenReturn(
                                Mono.just(validConfigurationJson),
                                Mono.just(validConfigurationWithChangedCbsConfigurationJson),
...
private val firstRequestDelay = Duration.ofSeconds(3)
private val differentFirstRequestDelay = Duration.ofSeconds(999)
...
private val validConfigurationWithChangedCbsConfigurationJson = JsonParser().parse("""
{
    "server.listenPort": 6061,
    "server.idleTimeoutSec": 10,
    "cbs.firstRequestDelaySec": ${differentFirstRequestDelay.seconds},
...
private val hvVesConfigurationWithChangedInterval = HvVesConfiguration(
        ...
        CbsConfiguration(differentFirstRequestDelay, updatedRequestInterval),
