...
class DcaeAppApiServer(private val simulator: DcaeAppSimulator) {
    private val validResponseMessage = "validation passed"
    private val invalidResponseMessage = "consumed messages don't match data from validation request"
    ...
    private fun setupHandlers(chain: Chain) {
        chain
                ...
                .post("messages/all/validate") { ctx ->
                    ctx.request.body.then { body ->
                        logger.info { "Processing request for received message validation" }
