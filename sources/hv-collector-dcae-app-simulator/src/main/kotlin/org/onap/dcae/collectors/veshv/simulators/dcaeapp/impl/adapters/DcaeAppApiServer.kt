...
class DcaeAppApiServer(private val simulator: DcaeAppSimulator) {
    ...
    private fun doWithTopicOrReturnInternalErrorResponse(req: HttpServerRequest, function: (String) -> Mono<Response>) =
            getTopicOption(req)
                    .fold({ Mono.just(Responses.InternalServerError) }, function)

    private fun getTopicOption(req: HttpServerRequest) =
