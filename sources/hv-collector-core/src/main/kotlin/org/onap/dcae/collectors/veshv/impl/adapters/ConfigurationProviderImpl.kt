...
internal class ConfigurationProviderImpl(private val cbsClientMono: Mono<CbsClient>,
    ...
    override fun invoke(): Flux<Stream<KafkaSink>> =
            cbsClientMono
    ...
    private fun handleUpdates(cbsClient: CbsClient): Flux<Stream<KafkaSink>> = cbsClient
    ...
    private fun createCollectorConfiguration(configuration: JsonObject): Stream<KafkaSink> {
        try {
            val sinks = DataStreams.namedSinks(configuration)
                    .groupBy { it.type() }
            return sinks.getOrElse("kafka", Stream.empty()).map(streamParser::unsafeParse)
