...
internal class ConsulConfigurationProvider(private val http: HttpAdapter,
    ...
    private fun askForConfig(): Mono<WithInvocationId<String>> = Mono.defer {
        val invocationId = UUID.randomUUID()
        http.get(url, invocationId).map { WithInvocationId(it, invocationId) }
    }

    private fun filterDifferentValues(configuration: WithInvocationId<String>) =
            configuration.value.let { configurationString ->
    ...
    private data class WithInvocationId<T>(val value: T, val invocationId: UUID)
}
