...
private val logger = Logger("org.onap.dcae.collectors.veshv.utils.http.httpserverresponse")

fun HttpServerResponse.sendOrError(action: IO<Unit>): NettyOutbound {
    return sendAndHandleErrors(action.map {
        Response(
            HttpStatus.OK,
            Content(
                ContentType.JSON,
...
fun HttpServerResponse.sendAndHandleErrors(response: IO<Response>): NettyOutbound {
    return response.attempt().unsafeRunSync().fold(
        { err ->
