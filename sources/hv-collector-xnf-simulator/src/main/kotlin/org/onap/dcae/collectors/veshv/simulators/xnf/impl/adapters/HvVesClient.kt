...
class HvVesClient(private val producer: HvVesProducer) {
    ...
    fun sendRawPayload(messages: Flux<ByteBuffer>, payloadType: PayloadType = PayloadType.UNDEFINED): Mono<Unit> {
        ...
        messages.count().subscribe { counter.addAndGet(it) }
        
        return producer.sendRaw(messages, payloadType)
                .then {
                    logger.info {
                        "Producer sent ${counter.get()} raw messages with payload type ${payloadType}"
                    }
                }
    }
