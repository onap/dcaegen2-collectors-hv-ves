...
class CommonEventHeaderParser {
    fun parse(json: JsonObject): CommonEventHeader {
        val headerBuilder = VesEventV5.VesEvent.CommonEventHeader.newBuilder()
        JsonFormat.parser().merge(
                json.toString(),
                headerBuilder)
        val commonEventHeader = headerBuilder.build()
        if(!isValid(commonEventHeader)) {
            throw IllegalStateException("Required field is missing from message header")
        }
