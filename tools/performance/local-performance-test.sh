...
function setup_environment(){
    ...
    while [[ $(docker ps | grep performance | grep -c unhelathy) -ne 0 ]] ; do
        sleep 1
    done
    echo "All component ready"
    exit 0
}
...
function start_performance_test(){
    ...
    for i in $(seq 1 ${CONTAINERS_COUNT}); do
        docker run -d --rm --network="$HV_VES_NETWORK" --volume "$VOLUME_MAPPING" "$PRODUCER_IMAGE_NAME" \
