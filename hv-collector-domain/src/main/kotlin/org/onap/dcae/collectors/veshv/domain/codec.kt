...
class WireFrameEncoder(private val allocator: ByteBufAllocator = ByteBufAllocator.DEFAULT) {
    ...
    fun encode(frame: WireFrameMessage): ByteBuf {
        val bb = allocator.buffer(WireFrameMessage.HEADER_SIZE + frame.payload.size())

        bb.writeByte(WireFrameMessage.MARKER_BYTE.toInt())
        bb.writeByte(frame.versionMajor.toInt())
        bb.writeByte(frame.versionMinor.toInt())
        bb.writeZero(RESERVED_BYTE_COUNT)
        bb.writeByte(frame.payloadType.toInt())
