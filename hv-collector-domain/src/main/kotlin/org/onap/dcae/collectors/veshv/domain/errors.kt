...
class InvalidMajorVersion(actualVersion: Short) : WireFrameMessageValidationError(
        "Invalid major version. Expected ${WireFrameMessage.SUPPORTED_VERSION_MAJOR} but was $actualVersion")

class UnsupportedPayloadContentType(actualType: Int) : WireFrameMessageValidationError(
        "Invalid start of frame. Expected one of ${PayloadContentType.hexValues}, but was $actualType")

class NotMatchingPayloadSize(definedInHeader: Int, actual: Int) :
        WireFrameMessageValidationError("Payload size does not match one defined in wire frame header.\n" +
