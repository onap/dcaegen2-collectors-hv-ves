...
function usage() {
    echo ""
    echo "Sent messages to hv-ves from multiple xNF simulators"
    echo "Usage: $0 [-h|--help] [-v|--verbose] [--messages-in-batch] [--docker-network]"
    ...
    echo "  - messages-in-batch : Amount of messages sent on each reaquest"
    ...
    echo "./start-simulation.sh --messages-in-batch=5 localhost 6061 10 20 0.5"
    exit 1
}
...
function create_xnfs_simulators(){
    for i in $(seq 1 ${XNFS_AMOUNT}); do
        XNF_PORT=$(get_unoccupied_port 32000 65000)
        verbose_log "Starting xNF container on port ${XNF_PORT} using run-xnf-simulator script"
...
function wait_for_containers_startup_or_fail() {
    ...
    if [ $all_containers_healthy -ne 1 ]; then
        echo "Some xnf containers failed at startup. Trying to cleanup..."
        cleanup
        echo "Exitting..."
        exit 2
    fi
...
function wait_for_simulators_to_finish_sending_messages() {
    ...
    echo "Waiting up to ${seconds_to_wait}s for xNF simulators to finish sending messages"
    for i in $(seq 1 ${seconds_to_wait}); do
...
# TODO: option to give path to xnfs logs directory?
# TODO: hv-ves network? create temp network for xnfs when ves is on remote?
...
echo "Creating ${XNFS_AMOUNT} xNF simulators"
...
for port in ${CREATED_XNF_SIMULATORS_PORTS[@]}; do
    start_simulation $port $MESSAGES_IN_BATCH &
done

# assure all xNFs request were sent
WAIT_TIME_FOR_REQUESTS_TO_BE_SENT=$(echo ";$MESSAGES_SENDING_INTERVAL * $MESSAGE_BATCHES_AMOUNT" | bc)
echo "Waiting ${WAIT_TIME_FOR_REQUESTS_TO_BE_SENT}s for all xNF requests to be sent"
sleep $WAIT_TIME_FOR_REQUESTS_TO_BE_SENT

wait_for_simulators_to_finish_sending_messages 60
